...
class DuplicateDetectionSmokeTest:
    ...
    async def test_hierarchy_mappings(self):
        ...
        for name, expected_parent, expected_level in test_cases:
            ...
            if mapping:
                ...
                self.log_result(f"'{name}' → {expected_parent} (level {expected_level})", False, "No mapping found")
    ...
    async def test_entity_type_similarity(self, session):
        ...
        result = await session.execute(select(EntityType).where(EntityType.is_active.is_(True)))
            ...
            if should_match:
                if matches:
                    ...
                    if expected_substring and expected_substring.lower() in match_name.lower():
                        self.log_result(f"'{test_name}' → '{match_name}' ({int(score * 100)}%)", True)
                    else:
                        self.log_result(
                            f"'{test_name}' should match '{expected_substring}'", False, f"Got '{match_name}' instead"
                        )
                else:
                    self.log_result(f"'{test_name}' should find match", False, "No matches found")
            else:
                if not matches:
                    self.log_result(f"'{test_name}' should NOT match", True)
                else:
                    self.log_result(f"'{test_name}' should NOT match", False, f"Unexpected match: {matches[0][0].name}")
    ...
    async def test_facet_type_similarity(self, session):
        ...
        result = await session.execute(select(FacetType).where(FacetType.is_active.is_(True)))
        existing_types = result.scalars().all()
    ...
    async def test_entity_matching_service(self, session):
        ...
        if test_entity:
            ...
            if duplicate and duplicate.id == test_entity.id:
                ...
                self.log_result("Exact duplicate returns same entity", False, "Created new entity instead")
            ...
            if variation and variation.id == test_entity.id:
                ...
                self.log_result("Normalized duplicate returns same entity", False, "Created new entity instead")
    ...
    async def test_relation_type_similarity(self, session):
        ...
        result = await session.execute(select(RelationType).where(RelationType.is_active.is_(True)))
        existing_types = result.scalars().all()
    ...
    async def test_location_duplicate_detection(self, session):
        ...
        result = await session.execute(select(Location).where(Location.is_active.is_(True)).limit(1))
    ...
    async def test_config_hash_functions(self):
        ...
        self.log_result("Same config produces same hash", hash1 == hash2, f"Hashes differ: {hash1} != {hash2}")

        # Test: Different config = different hash
        config3 = {"entity_type": "windpark", "filters": {"country": "AT"}}
        hash3 = compute_config_hash(config3)
        self.log_result(
            "Different config produces different hash", hash1 != hash3, f"Hashes should differ but both are {hash1}"
        )

        # Test: Order of keys doesn't matter
        config4 = {"filters": {"country": "DE"}, "entity_type": "windpark"}
        hash4 = compute_config_hash(config4)
        self.log_result("Key order doesn't affect hash", hash1 == hash4, f"Hashes differ: {hash1} != {hash4}")
        ...
        self.log_result("Nested config produces consistent hash", hash5 == hash6, f"Hashes differ: {hash5} != {hash6}")
    ...
    async def test_api_configuration_duplicate(self, session):
        ...
        result = await session.execute(
            select(APIConfiguration).options(selectinload(APIConfiguration.data_source)).limit(1)
        )
        ...
        self.log_result("Find existing by URL", duplicate is not None, "No duplicate found" if not duplicate else "")
