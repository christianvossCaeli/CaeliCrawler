...
async def export_usage_data(
    ...
    if format == "json":
        ...
        writer.writerow([
            ...
            "cost_cents", "cost_usd", "avg_duration_ms", "error_count", "error_rate"
        ])
        s = analytics.summary
        writer.writerow([
            s.total_requests, s.total_tokens, s.total_prompt_tokens, s.total_completion_tokens,
            s.total_cost_cents, f"{s.total_cost_cents / 100:.2f}", f"{s.avg_duration_ms:.1f}",
            s.error_count, f"{s.error_rate:.4f}"
        ])
        writer.writerow([])
        ...
        for trend in analytics.daily_trend:
            writer.writerow([
                trend.day.isoformat(), trend.request_count, trend.total_tokens,
                trend.cost_cents, f"{trend.cost_cents / 100:.2f}", trend.error_count
            ])
        writer.writerow([])
        ...
        for m in analytics.by_model:
            writer.writerow([
                m.model, m.provider.value, m.request_count, m.total_tokens,
                m.cost_cents, f"{m.cost_cents / 100:.2f}", f"{m.avg_tokens_per_request:.1f}"
            ])
        writer.writerow([])
        ...
        for t in analytics.by_task:
            writer.writerow([
                t.task_type.value, t.request_count, t.total_tokens,
                t.cost_cents, f"{t.cost_cents / 100:.2f}", f"{t.avg_duration_ms:.1f}"
            ])
        writer.writerow([])
        ...
        for c in analytics.by_category:
            writer.writerow([
                str(c.category_id) if c.category_id else "", c.category_name or "(uncategorized)",
                c.request_count, c.total_tokens, c.cost_cents, f"{c.cost_cents / 100:.2f}"
            ])
        writer.writerow([])
        ...
        writer.writerow([
            "user_id", "user_email", "user_name", "requests", "tokens",
            "prompt_tokens", "completion_tokens", "cost_cents", "cost_usd", "models_used", "has_credentials"
        ])
        for u in analytics.by_user:
            writer.writerow([
                str(u.user_id) if u.user_id else "(system)", u.user_email or "", u.user_name or "",
                u.request_count, u.total_tokens, u.prompt_tokens, u.completion_tokens,
                u.cost_cents, f"{u.cost_cents / 100:.2f}", ";".join(u.models_used), u.has_credentials
            ])
        writer.writerow([])
        ...
        for tc in analytics.top_consumers:
            writer.writerow([
                tc.task_name, tc.task_type.value, tc.request_count, tc.total_tokens,
                tc.cost_cents, f"{tc.cost_cents / 100:.2f}"
            ])
