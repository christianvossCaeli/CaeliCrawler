...
async def bulk_load_documents_with_sources(session: AsyncSession, doc_ids: set) -> dict[UUID, Document]:
...
async def bulk_load_categories(session: AsyncSession, cat_ids: set) -> dict[UUID, Category]:
    """Bulk-load categories to avoid N+1 queries."""
    if not cat_ids:
        return {}
    result = await session.execute(select(Category).where(Category.id.in_(cat_ids)))
    return {cat.id: cat for cat in result.scalars().all()}


async def bulk_load_sources(session: AsyncSession, source_ids: set) -> dict[UUID, DataSource]:
    """Bulk-load data sources to avoid N+1 queries."""
    if not source_ids:
        return {}
    result = await session.execute(select(DataSource).where(DataSource.id.in_(source_ids)))
