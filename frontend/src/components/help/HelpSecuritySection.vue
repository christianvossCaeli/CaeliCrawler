<template>
  <HelpCard section-id="security" :title="t('help.security.title')" icon="mdi-shield-lock" color="primary">
    ...
    <h3 class="text-h6 mb-3">{{ t('help.security.emailVerification.heading') }}</h3>
    <p class="mb-3">{{ t('help.security.emailVerification.description') }}</p>
      ...
      <v-col cols="12" md="6">
        <v-card variant="outlined" class="h-100">
          <v-card-title class="text-subtitle-1">
            ...
            {{ t('help.security.emailVerification.status.verified') }}
          </v-card-title>
          <v-card-text>
            {{ t('help.security.emailVerification.status.verifiedDesc') }}
          </v-card-text>
        </v-card>
      </v-col>
      <v-col cols="12" md="6">
        <v-card variant="outlined" class="h-100">
          <v-card-title class="text-subtitle-1">
            ...
            {{ t('help.security.emailVerification.status.pending') }}
          </v-card-title>
          <v-card-text>
            {{ t('help.security.emailVerification.status.pendingDesc') }}
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
    ...
    <v-expansion-panels variant="accordion" class="mb-4">
      <v-expansion-panel>
        <v-expansion-panel-title>
          ...
          {{ t('help.security.emailVerification.workflow.title') }}
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <v-list density="compact">
            <v-list-item prepend-icon="mdi-numeric-1-circle">
              <v-list-item-title>{{ t('help.security.emailVerification.workflow.step1') }}</v-list-item-title>
            </v-list-item>
            <v-list-item prepend-icon="mdi-numeric-2-circle">
              <v-list-item-title>{{ t('help.security.emailVerification.workflow.step2') }}</v-list-item-title>
            </v-list-item>
            <v-list-item prepend-icon="mdi-numeric-3-circle">
              <v-list-item-title>{{ t('help.security.emailVerification.workflow.step3') }}</v-list-item-title>
            </v-list-item>
          </v-list>
          <v-alert type="info" variant="tonal" density="compact" class="mt-2">
            {{ t('help.security.emailVerification.rateLimit') }}
          </v-alert>
        </v-expansion-panel-text>
      </v-expansion-panel>
      ...
      <v-expansion-panel>
        <v-expansion-panel-title>
          <v-icon class="mr-2" color="success">mdi-shield-star</v-icon>
          {{ t('help.security.emailVerification.benefits.title') }}
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <v-list density="compact">
            <v-list-item prepend-icon="mdi-lock-reset">
              <v-list-item-title>{{ t('help.security.emailVerification.benefits.passwordReset') }}</v-list-item-title>
            </v-list-item>
            <v-list-item prepend-icon="mdi-bell">
              <v-list-item-title>{{ t('help.security.emailVerification.benefits.notifications') }}</v-list-item-title>
            </v-list-item>
            <v-list-item prepend-icon="mdi-shield-check">
              <v-list-item-title>{{ t('help.security.emailVerification.benefits.security') }}</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-expansion-panel-text>
      </v-expansion-panel>
